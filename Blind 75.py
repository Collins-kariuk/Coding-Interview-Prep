...
def insert(intervals, newInterval):
    ...
    for i in range(len(intervals)):
        # when the end of the new interval is less than the start of the current interval, we add the new interval to the result and then add the rest of the intervals 
        if newInterval[1] < intervals[i][0]:
            res.append(newInterval)
            return res + intervals[i::]
        # when the start of the new interval is greater than the end of the current interval, we add the current interval to the result
        elif newInterval[0] > intervals[i][1]:
            res.append(intervals[i])
        # when the start of the new interval is less than or equal to the end of the current interval, we merge the intervals
        else:
