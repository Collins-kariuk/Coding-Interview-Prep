...
def longestConsecutive(nums):
    ...
    for n in nums:
        # check whether the number before the current number is the start of a new sequence if it
        # is, then we can calculate the length of that particular sequence we do this by checking
        # whether the number before the current number is in the set if it is, then we know that
        # the current number is not the start of a new sequence and we can safely skip it
        if n - 1 not in numSet:
            # variable that'll store the length of the current sequence
            length = 0
            # we calculate the length of the current sequence by checking whether the number after
            # the current number is in the set and if it is, we increment the length notice that
            # since we initialize length to 0, we ensure that we count the current start of the
            # sequence as well
...
def isValid(s):
    ...
    for char in s:
        ...
        if char in closeToOpen:
            # If the stack is not empty and the open parenthesis at the top of the stack matches
            # the corresponding open parenthesis for the current closing parenthesis, it indicates
            # that we have a matching pair of parentheses. In this case, we can remove the open
            # parenthesis from the top of the stack.
            ...
            # expected counterpart for ']'.
            else:
                return False
        # When char is an open parenthesis, we add it to the stack and it will be compared in a
        # later iteration
        else:
            stack.append(char)
    # The input string will be valid only if the stack is empty after iterating through the entire
    # string. This means that all matching parentheses have been successfully paired and removed,
